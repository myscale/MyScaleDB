...
async def importSIFT(id, insert_csv, search_data, tableName, query_num=10, k=50,  build=True, search=True,stress_test_time=0,aio_query=1):
   #base, query, train, truth = load_sift10M()
    ...
    os_client = "clickhouse-client --port 10000"
    time_start = time.time()
    s =  ClientSession()
    client = ChClient(s,url= 'http://localhost:10123/')
    while True:
        if build:
            print(id,"   start insert")
            time0 =time.time()
            insert_string = '{} -q \"INSERT INTO {} FORMAT CSV\" < {}'.format(os_client,tableName,insert_csv)
        ...
        if search:
            ...
            time0 =time.time()
            result = await client.fetch("SELECT id,data, batch_distance('topK={}','nprobe=1')(data, {}) from {}".format(k, search_data, tableName))
            #result = await client.fetch("SELECT id from {} where id%10000=1 limit 50".format(tableName))
            time1 = time.time()
            for x in result:
                print(x["id"])
            # print(result)
            print(id," search time spent: ", time1 - time0)
            #print(id," accuracy: ", evaluate(id, result, query[id*query_num:(id+1)*query_num], truth, k))
            aio_query -=1
            print(aio_query)
        if(time.time()-time_start>stress_test_time or aio_query == 0):
            break
    await s.close()
...
#def synchronize_async_helper(to_await,id, insert_csv, search_data, tableName,query_per_thread, k, insert, search,stress_test_time):
def synchronize_async_helper(to_await,*args):
    async_response = []

    async def run_and_capture_result():
        #r = await to_await(id,insert_csv, search_data, tableName,query_per_thread, k, insert, search,stress_test_time)
        r = await to_await(args[0],args[1],args[2],args[3])
...
def main():
    name = "profile_lock"
    metrics = "'metric_type = L2', 'compression_scheme=LZ4','mode = CPU','ef_c=100'"
    index_type = "IVFFLAT"
    thread_num=1
    insert_data_per_thread = 10000000
    k = 50
    query_per_thread=1
    aio_query_per_thread = 100
    ...
    if stress_test_time>0:
        ...
        t = threading.Thread(synchronize_async_helper(importSIFT,i,insert_file_name,search_data[i*query_per_thread:(i+1)*query_per_thread], name,query_per_thread, k, insert, search,stress_test_time))
