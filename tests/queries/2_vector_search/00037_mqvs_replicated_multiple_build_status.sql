...
select table, name, expr, status, latest_failed_part, if(position(latest_fail_reason,'ception')>0, substr(latest_fail_reason, position(latest_fail_reason,'ception') + 8), latest_fail_reason) from system.vector_indices where database = currentDatabase() order by table;
