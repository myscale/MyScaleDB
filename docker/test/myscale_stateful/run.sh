...
function start() {
    if [[ -n "$USE_DATABASE_REPLICATED" ]] && [[ "$USE_DATABASE_REPLICATED" -eq 1 ]]; then
        rm -rf /etc/clickhouse-server1/config.d/listen.xml
        echo '<clickhouse><listen_host>0.0.0.0</listen_host></clickhouse>' >>/etc/clickhouse-server1/config.d/listen.xml
        echo '<clickhouse><interserver_listen_host>0.0.0.0</interserver_listen_host></clickhouse>' >>/etc/clickhouse-server1/config.d/interserver_listen_host.xml

        rm -rf /etc/clickhouse-server2/config.d/listen.xml
        echo '<clickhouse><listen_host>0.0.0.0</listen_host></clickhouse>' >>/etc/clickhouse-server2/config.d/listen.xml
        echo '<clickhouse><interserver_listen_host>0.0.0.0</interserver_listen_host></clickhouse>' >>/etc/clickhouse-server2/config.d/interserver_listen_host.xml
...
function run_tests()
{
    ...
    clickhouse-test "${TEST_ARGS[@]}" --skip 00157_cache_dictionary 2>&1 | ts '%Y-%m-%d %H:%M:%S' | tee test_output/test_result.txt
    set -e
}
