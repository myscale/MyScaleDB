...
mkdir -p /etc/docker/
mkdir -p /ClickHouse/tests/integration/
echo '{
    "ipv6": true,
    ...
    "insecure-registries" : [
        "dockerhub-proxy.dockerhub-proxy-zone:5000",
        "harbor.internal.moqi.ai"
    ],
...
HARBOR_USER_NAME="$(echo $HARBOR_USER_B64 | base64 -d)"
HARBOR_USER_PASSWORD="$HARBOR_PASSWORD"

echo $HARBOR_USER_PASSWORD | docker login harbor.internal.moqi.ai/mqdb --username $HARBOR_USER_NAME --password-stdin ||:
