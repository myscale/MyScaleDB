...
class PartReader : public Search::IndexSourceDataReader<typename VectorSearchTypeMap<T>::IndexDatasetType>
{
public:
    using DataChunk = typename Search::IndexSourceDataReader<typename VectorSearchTypeMap<T>::IndexDatasetType>::DataChunk;
    ...
    std::shared_ptr<DataChunk> merge(const std::vector<std::shared_ptr<DataChunk>> &chunks)
    {
        ...
        typename VectorSearchTypeMap<T>::IndexDatasetType *data;
...
protected:
    std::shared_ptr<DataChunk> readDataImpl(size_t n) override
    {
        ...
        Search::idx_t * ids;
        typename VectorSearchTypeMap<T>::IndexDatasetType *vector_raw_data;
        size_t total_rows = 0;
        ...
        if constexpr (T == DB::VectorSearchType::Float32Vector)
        {
            ...
            if (!array)
            {
                throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Vector column type for Float32Vector is not Array in part {}", part->name);
            }
        ...
        else if constexpr (T == DB::VectorSearchType::BinaryVector)
        {
            if (const DB::ColumnFixedString *fixed_string = DB::checkAndGetColumn<DB::ColumnFixedString>(one_column.get()))
            {
                ...
                if (fixed_N * 8 != dimension)
                    throw DB::Exception(DB::ErrorCodes::INCORRECT_DATA, "Vector column for BinaryVector, N of FixedString(N) does not match dimension in part {}", part->name);
...
extern template class PartReader<DB::VectorSearchType::Float32Vector>;
extern template class PartReader<DB::VectorSearchType::BinaryVector>;
};
