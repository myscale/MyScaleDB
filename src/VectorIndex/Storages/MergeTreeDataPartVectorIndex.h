...
using VectorIndexDescriptionPtr = std::shared_ptr<const VectorIndexDescription>;
...
public:
    MergeTreeDataPartColumnIndex(
        ...
        const VectorIndexDescription & vec_desc_,
        const UInt64 rows_count_,
        StorageMetadataPtr metadata_snapshot,
        DataPartStoragePtr part_storage_,
        MergeTreeSettingsPtr merge_tree_setting);

    MergeTreeDataPartColumnIndex(const IMergeTreeDataPart & part, const VectorIndexDescription & vec_desc_);
    ...
    VectorIndexMetric getMetric() const { return Search::getMetricType(metric_str, vector_search_type); }
    ...
    void buildIndex(
        ...
        VectorIndexSourceDataReader<T> * part_reader,
        bool slow_mode,
    ...
    bool cache(VectorIndexVariantPtr index);
    ...
    static void transferToNewRowIds(const VectorIndexWithMeta & index_with_meta, SearchResultPtr result);

    static SearchResultPtr TransferToOldRowIds(const VectorIndexWithMeta & index_with_meta, const SearchResultPtr result);
    ...
    static bool supportTwoStageSearch(const VectorIndexWithMeta & index_with_meta);
...
private:
    VectorIndexVariantPtr createIndex() const;
    ...
    std::function<bool()> cancel_build_callback)
{
    ...
    {
        if (!std::holds_alternative<FloatVectorIndexPtr>(index_variant))
            throw IndexException(ErrorCodes::LOGICAL_ERROR, "Index Type error, except parameter type to be FloatVector");

        index_ptr = std::get<FloatVectorIndexPtr>(index_variant);
    }
    else if constexpr (T == Search::DataType::BinaryVector)
    {
        if (!std::holds_alternative<BinaryVectorIndexPtr>(index_variant))
            throw IndexException(ErrorCodes::LOGICAL_ERROR, "Index Type error, except parameter type to be BinaryVector");

        index_ptr = std::get<BinaryVectorIndexPtr>(index_variant);
    }
    else
        throw IndexException(ErrorCodes::LOGICAL_ERROR, "Unknown Serach Type!");
...
public:
    ...
    void addVectorIndex(const VectorIndexDescription & vec_desc);

    void removeVectorIndex(const String & index_name);

    std::optional<MergeTreeDataPartColumnIndexPtr> getColumnIndex(const VectorIndexDescription & vec_desc)
    {
