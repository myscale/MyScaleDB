...
namespace DB
{
VectorIndexTaskBase::State VectorIndexTaskBase::getNextState()
{
    if (build_status.getStatus() == BuildVectorIndexStatus::SUCCESS && state != VectorIndexTaskBase::State::NEED_FINALIZE)
        return static_cast<VectorIndexTaskBase::State>(static_cast<int>(state) + 1);
    else if (build_status.getStatus() == BuildVectorIndexStatus::BUILD_RETRY)
        return state;
    else
        /// build error
        return VectorIndexTaskBase::State::NEED_FINALIZE;
}

void VectorIndexTaskBase::recordBuildStatus()
{
    ...
    VectorIndexDescription vec_desc;
    ...
    bool is_success
        = build_status.getStatus() == BuildVectorIndexStatus::SUCCESS || build_status.getStatus() == BuildVectorIndexStatus::BUILD_SKIPPED;
    ...
    if (ctx)
    {
        if (ctx->source_column_index->isShutdown() && build_status.getStatus() != BuildVectorIndexStatus::BUILD_CANCEL)
        ...
        if (is_success)
        {
            ...
            ctx->write_event_log(VectorIndexEventLogElement::BUILD_SUCCEED, 0, "");
            state = State::SUCCESS;
        }
        else
        {
            ...
            VectorIndexEventLogElement::Type event_type = build_status.getStatus() == BuildVectorIndexStatus::BUILD_CANCEL
                ? VectorIndexEventLogElement::BUILD_CANCELD
...
bool VectorIndexTaskBase::executeStep()
{
    switch (state)
    {
        ...
        case State::NEED_EXECUTE_BUILD_VECTOR_INDEX: {
            build_status = builder.buildVectorIndex(ctx);
        ...
        case State::NEED_MOVE_VECTOR_INDEX: {
            try
            {
                build_status = builder.TryMoveVectorIndexFiles(ctx);
            }
            catch (Exception & e)
            {
                LOG_WARNING(
                    ...
                    e.code(),
                    e.message());
                build_status = BuildVectorIndexStatus{BuildVectorIndexStatus::BUILD_FAIL, e.code(), e.message()};
            }
...
StorageID VectorIndexTaskBase::getStorageID()
{
    return storage.getStorageID();
}

void VectorIndexTaskBase::onCompleted()
{
...
VectorIndexTaskBase::~VectorIndexTaskBase()
{
}
