...
{
public:
    VectorIndexBuilderUpdater(MergeTreeData & data_);
    ...
    void checkPartCanBuildVectorIndex(MergeTreeDataPartPtr data_part, const VectorIndexDescription & vec_desc);

    void removeDroppedVectorIndices(const StorageMetadataPtr & metadata_snapshot);

    /// handle build index task
    BuildVectorIndexStatus buildVectorIndex(const VectorIndexContextPtr ctx);
    
    VectorIndexContextPtr prepareBuildIndexContext(
        const StorageMetadataPtr & metadata_snapshot,
    ...
    BuildVectorIndexStatus TryMoveVectorIndexFiles(const VectorIndexContextPtr ctx);
...
private:
    ...
    buildVectorIndexForOnePart(VectorIndexContextPtr ctx);
    ...
    void moveVectorIndexFilesToFuturePartAndCache(
