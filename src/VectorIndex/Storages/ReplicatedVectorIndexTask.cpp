...
BuildVectorIndexStatus ReplicatedVectorIndexTask::prepare()
{
    ...
    {
        ctx.reset();
        ctx = builder.prepareBuildIndexContext(metadata_snapshot, entry.source_parts.at(0), entry.index_name, slow_mode, true);
    }
    catch (Exception & e)
    {
        LOG_ERROR(&Poco::Logger::get("VectorIndexTask"), "Prepare build vector index {} error {}: {}", part_name, e.code(), e.message());
...
void ReplicatedVectorIndexTask::remove_processed_entry()
{
    try
    {
        ...
        if (replicated_storage.getSettings()->build_vector_index_on_random_single_replica)
        {
            ...
            if (build_status.getStatus() == BuildVectorIndexStatus::NO_DATA_PART
                || build_status.getStatus() == BuildVectorIndexStatus::BUILD_SKIPPED)
