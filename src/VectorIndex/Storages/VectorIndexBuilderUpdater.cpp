...
VectorIndexBuilderUpdater::VectorIndexBuilderUpdater(MergeTreeData & data_)
...
void VectorIndexBuilderUpdater::removeDroppedVectorIndices(const StorageMetadataPtr & metadata_snapshot)
{
    ...
    std::list<std::pair<VectorIndex::CacheKey, VectorIndexParameter>> cached_item_list = VectorIndex::CacheManager::getAllCacheNames();
    ...
    for (const auto & cache_item : cached_item_list)
    {
        ...
        if (!clear_cache)
        {
            ...
            VectorIndexParameter params = cache_item.second;
...
bool VectorIndexBuilderUpdater::allowToBuildVectorIndex(const bool slow_mode, const size_t builds_count_in_queue) const
{
    ...
    if (slow_mode)
    {
        ...
        if (builds_count_in_queue >= server_settings.background_slow_mode_vector_pool_size)
        {
            LOG_DEBUG(
                log,
                "[allowToBuildVectorIndex] Number of queued build vector index enties ({})"
...
VectorIndexEntryPtr
VectorIndexBuilderUpdater::selectPartToBuildVectorIndex(const StorageMetadataPtr & metadata_snapshot, bool select_slow_mode_part)
{
...
void VectorIndexBuilderUpdater::checkPartCanBuildVectorIndex(
    MergeTreeDataPartPtr data_part, const VectorIndexDescription & vec_desc)
{
    if (!data_part)
...
VectorIndexContextPtr VectorIndexBuilderUpdater::prepareBuildIndexContext(
...
{
    auto ctx = std::make_shared<VectorIndexContext>();
    if (part_name.empty())
    ...
    for (auto & vec_index_desc : metadata_snapshot->getVectorIndices())
    {
        ...
        checkPartCanBuildVectorIndex(part, vec_index_desc);
    ...
    ctx->log = &Poco::Logger::get("VectorIndexTask[" + part_name_prefix + "]");
    ...
    ctx->index_build_memory_lock = std::make_unique<IndexBuildMemoryUsageHelper>();
    ...
    ctx->write_event_log = [part = ctx->source_part, vector_index_name = ctx->vector_index_name](
                               VectorIndexEventLogElement::Type event_type, int error_code, const String & error_message) mutable
    {
...
BuildVectorIndexStatus VectorIndexBuilderUpdater::buildVectorIndex(VectorIndexContextPtr ctx)
{
    try
    {
        ...
        BuildVectorIndexStatus status = buildVectorIndexForOnePart(ctx);

        if (status.getStatus() == BuildVectorIndexStatus::SUCCESS)
            LOG_INFO(
                log,
...
BuildVectorIndexStatus VectorIndexBuilderUpdater::buildVectorIndexForOnePart(const VectorIndexContextPtr ctx)
{
    if (ctx->build_cancel_callback())
        return BuildVectorIndexStatus{BuildVectorIndexStatus::BUILD_CANCEL};
    ...
    {
        LOG_WARNING(log, "Found column {} in part and VectorIndexDescription, but not in metadata snapshot.", ctx->vec_index_desc.column);
        return BuildVectorIndexStatus{BuildVectorIndexStatus::META_ERROR};
    }
    ...
    if (part_storage == nullptr)
        return BuildVectorIndexStatus{BuildVectorIndexStatus::BUILD_FAIL};
    ...
    try
    {
        ctx->write_event_log(VectorIndexEventLogElement::BUILD_START, 0, "");

        /// variant index_ptr
        VectorIndexVariantPtr search_index;
        ...
        switch (search_type)
        {
            case Search::DataType::FloatVector:
            {
                ...
                VectorIndex::IndexReadFromPart<Search::DataType::FloatVector> part_reader(
                    ctx->source_part,
                    cols,
            ...
            case Search::DataType::BinaryVector:
            {
                ...
                VectorIndex::IndexReadFromPart<Search::DataType::BinaryVector> part_reader(
                    ctx->source_part,
                    cols,
                    ctx->metadata_snapshot,
                    data.getContext()->getMarkCache().get(),
    ...
    catch (Exception & e)
    {
        ...
        return BuildVectorIndexStatus{BuildVectorIndexStatus::BUILD_RETRY};
    }
...
BuildVectorIndexStatus VectorIndexBuilderUpdater::TryMoveVectorIndexFiles(const VectorIndexContextPtr ctx)
{
...
void VectorIndexBuilderUpdater::moveVectorIndexFilesToFuturePartAndCache(
    const MergeTreeDataPartPtr & dest_part,
    DiskPtr disk,
    VectorIndexVariantPtr build_index,
    ...
    const std::shared_ptr<MergeTreeDataPartChecksums> & /*checksums*/)
{
    ...
    if (!found_vector_file)
    {
        ...
        if (disk->exists(vector_tmp_relative_path))
            ...
            LOG_WARNING(
                log, "[moveVectorIndexFilesToFuturePartAndCache] vector_tmp_relative_path doesn't exist {}", vector_tmp_relative_path);
