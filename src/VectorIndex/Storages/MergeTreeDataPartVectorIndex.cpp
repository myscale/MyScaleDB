...
void MergeTreeDataPartColumnIndex::transferToNewRowIds(const VectorIndexWithMeta & index_with_meta, SearchResultPtr result)
{
...
SearchResultPtr MergeTreeDataPartColumnIndex::TransferToOldRowIds(const VectorIndexWithMeta & index_with_meta, const SearchResultPtr result)
{
...
bool MergeTreeDataPartColumnIndex::supportTwoStageSearch(const VectorIndexWithMeta & index_with_meta)
{
...
MergeTreeDataPartColumnIndex::MergeTreeDataPartColumnIndex(const IMergeTreeDataPart & part, const VectorIndexDescription & vec_desc)
...
MergeTreeDataPartColumnIndex::MergeTreeDataPartColumnIndex(
    ...
    const VectorIndexDescription & vec_desc,
...
void MergeTreeDataPartColumnIndex::shutdown()
{
    ...
    {
        CacheManager::removeFromCache(segment_id.getOriginalCacheKey());
        CacheManager::removeFromCache(segment_id.getCacheKey());
    }
}
...
void MergeTreeDataPartColumnIndex::initVectorIndexInfo(bool is_small_part)
{
    ...
    if (!segment_ids.empty() && !segment_ids[0].fromMergedParts())
    {
        ...
        std::lock_guard lock(vector_info_mutex);
        build_index_info->state = VectorIndexState::BUILT;
        vector_infos.emplace_back(std::make_shared<VectorIndexInfo>(
    ...
    else
    {
        ...
        vector_infos.emplace_back(
            std::make_shared<VectorIndexInfo>(storage_id.database_name, storage_id.table_name, *this, build_index_info->state));
        for (auto & segment_id : segment_ids)
        {
            std::lock_guard lock(vector_info_mutex);
...
VectorIndexVariantPtr MergeTreeDataPartColumnIndex::createIndex() const
{
    auto metric = Search::getMetricType(metric_str, vector_search_type);
    VectorIndexVariantPtr index_variant;
    VectorIndexParameter index_des = des;
    if (fallback_to_flat)
        std::erase_if(
            index_des,
            [](const auto & item)
    ...
    if (vector_search_type == Search::DataType::FloatVector)
        index_variant = Search::createVectorIndex<VectorIndexIStream, VectorIndexOStream, VectorIndexBitmap, VectorIndexDataType::FloatVector>(
            index_name,
            index_type,
            metric,
            dimension,
        ...
        index_variant = Search::createVectorIndex<VectorIndexIStream, VectorIndexOStream, VectorIndexBitmap, VectorIndexDataType::BinaryVector>(
            index_name,
            index_type,
            metric,
            dimension,
    ...
    IndexBuildMemoryUsageHelper & /*build_memory_lock*/)
{
    ...
    auto metric = Search::getMetricType(metric_str, vector_search_type);
    Metadata metadata(segment_id, version, index_type, metric, dimension, total_vec, fallback_to_flat, index_des, infos);
...
IndexWithMetaHolderPtr MergeTreeDataPartColumnIndex::loadDecoupleCache(SegmentId & segment_id)
{
    ...
    if (ori_index_holder)
    {
        ...
        auto load_func = [new_cache_item = new_cache_item, segment_id = segment_id]() mutable -> VectorIndexWithMetaPtr
        {
            ...
            auto delete_bitmap = std::make_shared<VectorIndexBitmap>(new_cache_item->total_vec, true);
...
IndexWithMetaHolderPtr MergeTreeDataPartColumnIndex::load(SegmentId & segment_id, bool is_active, const String & nvme_cache_path_uuid)
{
    ...
    auto write_event_log = [&](VectorIndexEventLogElement::Type event, int code = 0, String msg = "")
    {
    ...
    if (!index_holder)
    {
        ...
        write_event_log(VectorIndexEventLogElement::LOAD_START);
        auto load_func = [&]() -> VectorIndexWithMetaPtr
        {
            ...
            VectorIndexType load_index_type = index_type;
            VectorIndexParameter index_params = des;
            ...
            auto delete_bitmap = std::make_shared<VectorIndexBitmap>(index_total_vec, true);
            ...
            return std::make_shared<VectorIndexWithMeta>(
                index_variant,
                index_total_vec,
                delete_bitmap,
                index_params,
        ...
        try
        {
            ...
            if (!index_holder)
            {
                LOG_WARNING(
                    ...
                    index_name,
                    cache_key.toString());
                write_event_log(VectorIndexEventLogElement::LOAD_FAILED);
            }
            else
            {
                write_event_log(VectorIndexEventLogElement::LOAD_SUCCEED);
            }
        }
        catch (const Exception & e)
        {
            ...
            write_event_log(VectorIndexEventLogElement::LOAD_ERROR, e.code(), e.message());
            return nullptr;
        }
    }
...
bool MergeTreeDataPartColumnIndex::cache(VectorIndexVariantPtr index)
{
    auto segment_id = getCurrentFakeSegmentId();
    bool isNullptr = true;
    std::visit(
    ...
    auto delete_bitmap = std::make_shared<VectorIndexBitmap>(total_vec, true);
    ...
    VectorIndexWithMetaPtr cache_item = std::make_shared<VectorIndexWithMeta>(
...
SearchResultPtr MergeTreeDataPartColumnIndex::computeTopDistanceSubset(
    const VectorIndexWithMeta & index_with_meta, VectorDatasetVariantPtr queries, SearchResultPtr first_stage_result, int32_t top_k)
{
    auto index_variant = index_with_meta.index;
    if (vector_search_type != Search::DataType::FloatVector)
        throw IndexException(ErrorCodes::LOGICAL_ERROR, "Only Float32 Vector support two stage vector search");

    if (!std::holds_alternative<FloatVectorIndexPtr>(index_variant) || !std::holds_alternative<Float32VectorDatasetPtr>(queries))
        throw IndexException(ErrorCodes::LOGICAL_ERROR, "Vector index type and dataset type do not match search type");
...
SearchResultPtr MergeTreeDataPartColumnIndex::search(
    ...
    int32_t k,
    VectorIndexBitmapPtr filter,
    VectorIndexParameter & parameters,
    bool first_stage_only)
{
    // Check if the index is initialized and ready for searching
    auto & index_variant = index_with_meta.index;
    if ((vector_search_type == Search::DataType::FloatVector && std::holds_alternative<FloatVectorIndexPtr>(index_variant)
         && std::holds_alternative<Float32VectorDatasetPtr>(queries))
        || (vector_search_type == Search::DataType::BinaryVector && std::holds_alternative<BinaryVectorIndexPtr>(index_variant)
        ...
        std::visit(
            [&](auto && index_ptr)
            {
                if (index_ptr == nullptr)
                    throw IndexException(ErrorCodes::LOGICAL_ERROR, "Index not initialized before searching!");
                if (!index_ptr->ready())
                    throw IndexException(ErrorCodes::LOGICAL_ERROR, "Index not ready before searching!");
            },
            index_variant);
        ...
        std::visit(
            [&](auto && query_dataset)
            {
                ...
                if (query_dataset->getDimension() != static_cast<int64_t>(dimension))
                    throw IndexException(ErrorCodes::LOGICAL_ERROR, "The dimension of searched index and input doesn't match.");
            },
            queries);
    }
    else
    ...
    try
    {
...

        {
            ...
            if (vector_search_type == Search::DataType::FloatVector)
            {
                ...
                const FloatVectorIndexPtr & float_index = std::get<FloatVectorIndexPtr>(index_variant);
            ...
            else if (vector_search_type == Search::DataType::BinaryVector)
            {
                ...
                const BinaryVectorIndexPtr & binary_index = std::get<BinaryVectorIndexPtr>(index_variant);
    ...
    {
        throw IndexException(ErrorCodes::STD_EXCEPTION, e.what());
    }
}
...
void MergetreeDataPartVectorIndex::addVectorIndex(const VectorIndexDescription & vec_desc)
{
