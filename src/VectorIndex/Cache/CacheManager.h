...
{
public:
    size_t operator()(const VectorIndexWithMeta & index_meta) const;
};

class IndexWithMetaReleaseFunction
{
public:
    void operator()(std::shared_ptr<VectorIndexWithMeta> index_meta_ptr);
};

using VectorIndexCacheType
    = DB::LRUResourceCache<CacheKey, VectorIndexWithMeta, IndexWithMetaWeightFunc, IndexWithMetaReleaseFunction, std::hash<CacheKey>>;
using IndexWithMetaHolderPtr = VectorIndexCacheType::MappedHolderPtr;

class VectorIndexCache
    : public DB::LRUResourceCache<CacheKey, VectorIndexWithMeta, IndexWithMetaWeightFunc, IndexWithMetaReleaseFunction, std::hash<CacheKey>>
{
public:
...
public:
    ...
    std::list<std::pair<CacheKey, VectorIndexParameter>> getAllItems();

    static CacheManager * getInstance();
    static void setCacheSize(size_t size_in_bytes);

    static std::list<std::pair<CacheKey, VectorIndexParameter>> getAllCacheNames();
