...
struct VectorIndexWithMeta
{
    ...
    VectorIndexWithMeta() = delete;

    VectorIndexWithMeta(
        VectorIndexVariantPtr & index_,
        uint64_t total_vec_,
        VectorIndexBitmapPtr delete_bitmap_,
        VectorIndexParameter des_,
...
private:
    VectorIndexBitmapPtr delete_bitmap;
...
public:
    ...
    void setDeleteBitmap(VectorIndexBitmapPtr delete_bitmap_) { std::atomic_store(&delete_bitmap, std::move(delete_bitmap_)); }

    VectorIndexBitmapPtr getDeleteBitmap() const { return std::atomic_load(&delete_bitmap); }

    VectorIndexWithMetaPtr clone()
    {
