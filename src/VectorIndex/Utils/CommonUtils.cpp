...
void checkSparseSearchColumnDataType(DataTypePtr & data_type)
{
    ...
    if (map_type)
    {
        ...
        {
            throw Exception(ErrorCodes::INCORRECT_DATA, "The type of the key and value in the Map must be `UInt32` and `Float32` for sparse search column");
        }
    }
    else if (const DataTypeArray * array_type = typeid_cast<const DataTypeArray *>(data_type.get()))
    {
        /// TODO by libaoy
        /// only for develop
    }
