...
#include <VectorIndex/Processors/VectorScanSplitTransform.h>
...
VectorScanSplitTransform::VectorScanSplitTransform(
        const Block & header,
        size_t num_outputs,
        UInt64 limit_)
...
IProcessor::Status VectorScanSplitTransform::prepare()
{
...
IProcessor::Status VectorScanSplitTransform::prepareConsume()
{
...
IProcessor::Status VectorScanSplitTransform::prepareGenerate()
{
...
void VectorScanSplitTransform::splitChunk(Chunks result_chunks)
{
    DB::OpenTelemetry::SpanHolder span("VectorScanSplitTransform::splitChunk()");
    ...
    LOG_DEBUG(&Poco::Logger::get("VectorScanSplitTransform"), "Vector Scan Split {} rows to {} output ports", prev_split_rows, out_ports_data.size());
}
