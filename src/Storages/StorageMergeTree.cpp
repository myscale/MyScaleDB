    ...
    SelectPartsDecision * select_decision_out)
{
    ...
    auto can_merge = [this, &lock, &metadata_snapshot](const DataPartPtr & left, const DataPartPtr & right, const MergeTreeTransaction * tx, String * disable_reason) -> bool
    {
        ...
        return canMergeForVectorIndex(metadata_snapshot, left, right);
    };
    ...
    merging_tagger = std::make_unique<CurrentlyMergingPartsTagger>(
        future_part, MergeTreeDataMergerMutator::estimateNeededDiskSpace(future_part->parts, true), *this, metadata_snapshot, false);
    ...
    for (const auto & part : getDataPartsVectorForInternalUsage())
    {
        ...
        if (!commands->empty())
        {
            ...
            tagger = std::make_unique<CurrentlyMergingPartsTagger>(
                future_part, MergeTreeDataMergerMutator::estimateNeededDiskSpace({part}, false), *this, metadata_snapshot, true);
