...
/** Column, that is just group of few another columns. This column is same as Tuples.
  *
  * For constant Tuples, see ColumnConst.
  * Mixed constant/non-constant columns is prohibited in tuple
  *  for implementation simplicity.
  */
class ColumnObjectToFetch final : public COWHelper<IColumn, ColumnObjectToFetch>
{
private:
    friend class COWHelper<IColumn, ColumnObjectToFetch>;

    using ObjectToFetchColumns = std::vector<WrappedPtr>;
    ObjectToFetchColumns columns;

    template <bool positive>
    struct Less;

    explicit ColumnObjectToFetch(MutableColumns && columns);
    ColumnObjectToFetch(const ColumnObjectToFetch &) = default;

public:
    /** Create immutable column using immutable arguments. This arguments may be shared with other columns.
      * Use IColumn::mutate in order to make mutable column and mutate shared nested columns.
      */
    using Base = COWHelper<IColumn, ColumnObjectToFetch>;
    static Ptr create(const Columns & columns);
    static Ptr create(const ObjectToFetchColumns & columns);
    ...
    TypeIndex getDataType() const override { return TypeIndex::ObjectToFetch; }
    ...
    size_t ObjectToFetchSize() const { return columns.size(); }

    const IColumn & getColumn(size_t idx) const { return *columns[idx]; }
    IColumn & getColumn(size_t idx) { return *columns[idx]; }

    const ObjectToFetchColumns & getColumns() const { return columns; }
    ...
    {
        return getNumberOfDefaultRowsImpl<ColumnObjectToFetch>();
    }
