...
void InterpreterSelectQuery::executeFetchColumns(QueryProcessingStage::Enum processing_stage, QueryPlan & query_plan)
{
    ...
    else if (storage)
    {
        ...
        if (analysis_result.need_sparse_search)
        {
            ...
            String query_sparse_vector = "{";
            for (auto x : query_info.sparse_search_info->query_sparse_vector)
            {
                query_sparse_vector += "[" + std::to_string(x.first) + ": " + std::to_string(x.second) + "], ";
            }
            query_sparse_vector += "}";

            LOG_INFO(
                &Poco::Logger::get("InterpreterSelectQuery"),
                "sparse search info:\n"
                "sparse_column_name: {}\n"
                "query_sparse_vector column name: {}\n"
                "function_column_name: {}\n"
                "topk: {}\n"
                "search_mode: {}",
                query_info.sparse_search_info->sparse_column_name,
                query_sparse_vector,
                query_info.sparse_search_info->function_column_name,
                query_info.sparse_search_info->topk,
                query_info.sparse_search_info->getSparseSearchMode());
        }
